 ...
 * 2.1 网络状态回调
 *
 * @param status 网络状态，请参考[TIMNetworkStatus]()
 ...
 * 2.4 当前用户的资料更新的回调
 *
 * @param json_user_profile 当前用户的资料，请参考[UserProfile]()
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 */
 ...
 * 2.5 用户状态变更的回调
 *
 * @param json_user_status_array 用户状态的列表，用户状态请参考[TIMUserStatus]()
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 */
 ...
 * 2.6 订阅用户资料变更的回调
 *
 * @param json_user_info_array 订阅用户资料更新列表，用户资料请参考[UserProfile]()
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 */
 ...
 * 2.7 日志回调
 *
 * @param level 日志级别,请参考[TIMLogLevel]()
 * @param log 日志字符串
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 */
 ...
 * 3.1 设置网络连接状态监听回调
 * 
 * @param cb 连接事件回调，请参考[TIMNetworkStatusListenerCallback]()
 ...
 * 3.2 设置被踢下线通知回调
 * 
 * @param cb 踢下线回调，请参考[TIMKickedOfflineCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.3 设置票据过期回调
 * 
 * @param cb 票据过期回调，请参考[TIMUserSigExpiredCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.4 设置当前用户资料更新回调
 * 
 * @param cb 当前用户的资料更新的回调，请参考[TIMSelfInfoUpdatedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 3.5 设置用户状态变更回调
 * 
 * @param cb 用户状态变更的回调，请参考[TIMUserStatusChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note 收到通知的情况：
 ...
 * 3.6 设置用户资料变更回调
 * 
 * @param cb 用户资料更新的回调，请参考[TIMUserInfoChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.7 设置日志回调
 * 
 * @param cb 日志回调，请参考[TIMLogCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * @param json_sdk_config ImSDK配置选项Json字符串,详情请参考[SdkConfig]()
 ...
 * @param cb 返回设置配置之后所有配置的回调，此回调cb可为空，表示不获取所有配置信息。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 * - 目前支持设置的配置有http代理的IP和端口、socks5代理的IP和端口、输出日志的级别等。
 ...
 * @param cb 登录成功与否的回调。票据过期会返回 ERR_USER_SIG_EXPIRED（6206）或者 ERR_SVR_ACCOUNT_USERSIG_EXPIRED（70001） 错误码，此时请您生成新的 userSig 重新登录。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * 5.2 登出
 *
 * @param cb 登出成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * 5.4 获取登录状态
 *
 * @return TIMLoginStatus 每个返回值的定义请参考 [TIMLoginStatus]()
 *
 * @note
 ...
 * @param cb 获取指定用户列表的用户资料成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 ...
 * @param cb 修改自己的资料成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 订阅用户资料成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note 请注意：
 ...
 * @param cb 查询用户状态成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note 请注意：
 ...
 * @param cb 设置自己的状态成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note 请注意：
 * 该接口只支持设置自己的自定义状态，即 kTIMUserStatusCustomStatus 字段，请参考 [TIMUserStatus]()
 *
 * @example
 ...
 * @param cb 订阅用户状态成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note 请注意：
...
//------------------------------------------------------------------------------
// 7.6 SetConfig (更新配置)
// uint [TIMLogLevel](), 只写(选填), 输出到日志文件的日志级别
static const char* kTIMSetConfigLogLevel = "set_config_log_level";
// uint [TIMLogLevel](), 只写(选填), 日志回调的日志级别
...
// object [HttpProxyInfo](), 只写(选填), 设置HTTP代理，如果需要，在发送图片、文件、语音、视频前设置
// @note
...
// object [Socks5ProxyInfo](), 只写(选填), 设置SOCKS5代理，如果需要，初始化前设置
// @note
...
// object [PACProxyInfo](), 只写(选填), 设置PAC代理，如果需要，初始化前设置
...
// uint [TIMUserStatusType](), 只读, 用户的状态
...
// uint [TIMProfileAddPermission](), 只读, 用户加好友的选项
...
// array [UserProfileCustomStringInfo](), 只读, 请参考[自定义资料字段](https://cloud.tencent.com/document/product/269/1500#.E8.87.AA.E5.AE.9A.E4.B9.89.E8.B5.84.E6.96.99.E5.AD.97.E6.AE.B5)
...
// uint [TIMProfileAddPermission](), 只写, 修改用户加好友的选项
...
// array [UserProfileCustomStringInfo](), 只写, 修改[自定义资料字段](https://cloud.tencent.com/document/product/269/1500#.E8.87.AA.E5.AE.9A.E4.B9.89.E8.B5.84.E6.96.99.E5.AD.97.E6.AE.B5)
...
// string, 只写, 用户自定义资料字段的key值（包含前缀Tag_Profile_Custom_）
...
// uint [TIMProfileChangeType](), 只读, 资料变更类型
...
// object [UserProfileItem](), 只读, 具体的变更信息，只有当 kTIMProfileChangeElemChangeType 为 kTIMProfileChange_Profile 时有效
...
// uint64 [TIMGroupGetInfoFlag](), 读写(选填), 根据想要获取的信息过滤，默认值为0xffffffff(获取全部信息)
