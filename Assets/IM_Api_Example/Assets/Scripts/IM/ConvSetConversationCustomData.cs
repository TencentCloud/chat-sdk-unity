...
public class ConvSetConversationCustomData : MonoBehaviour
{
  public Text Header;
  ...
  void Start()
  {
    ...
    Submit.onClick.AddListener(ConvSetConversationCustomDataSDK);
  ...
  void ConvSetConversationCustomDataSDK()
  {
    if (ConvList.Count == 0)
    {
      return;
    ...
    TIMResult res = TencentIMSDK.ConvSetConversationCustomData(new List<string> {
      conv_id
    }, CustomData.text, Utils.addAsyncStringDataToScreen(GetResult));
