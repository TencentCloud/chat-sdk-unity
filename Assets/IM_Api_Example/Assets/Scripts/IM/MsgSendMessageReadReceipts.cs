...
public class MsgSendMessageReadReceipts : MonoBehaviour
{
  ...
  void HandleMsgListRes(params object[] parameters)
  {
    try
    {
      ...
      List<Message> receiptList = new List<Message>();
      foreach (Message item in msg_list)
      {
        if ((item.message_is_from_self == false) && (item.message_need_read_receipt == true))
        {
          print(item.message_msg_id);
          receiptList.Add(item);
        }
      }
      MsgSendMessageReadReceiptsSDK(receiptList);
    }
    catch (Exception ex)
    {
