...
public class CommunityModifyTopicPermissionInPermissionGroup : MonoBehaviour
{
  public Text Header;
  ...
  void Start()
  {
    ...
    Submit.onClick.AddListener(CommunityModifyTopicPermissionInPermissionGroupSDK);
  ...
  void CommunityModifyTopicPermissionInPermissionGroupSDK()
  {
    TopicPermission  topicPermission = new TopicPermission();
    topicPermission.topic_permission_key = TopicID.text;
    topicPermission.topic_permission_value = 7;

    List<TopicPermission> topicPermissionList = new List<TopicPermission>();
    topicPermissionList.Add(topicPermission);

    TIMResult res = TencentIMSDK.CommunityModifyTopicPermissionInPermissionGroup(GroupID.text, PermissionGroupID.text, topicPermissionList, Utils.addAsyncStringDataToScreen(GetResult));
