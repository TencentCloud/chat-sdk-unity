...
public class GroupDeleteGroupAttributes : MonoBehaviour
{
  string[] Labels = new string[] { "GroupIDLabel", "GroupCustomKeyLabel", "CustomKeyPlaceHolder" };
  public Text Header;
  ...
  void Start()
  {
    ...
    Submit.onClick.AddListener(GroupDeleteGroupAttributesSDK);
  ...
  void GroupDeleteGroupAttributesSDK()
  {
    if (groupIDList.Count < 1) return;
    var keyList = string.IsNullOrEmpty(CustomKey.text) ? null : new List<string>(CustomKey.text.Split(','));
    TIMResult res = TencentIMSDK.GroupDeleteGroupAttributes(groupIDList[SelectedGroup.value], keyList, Utils.addAsyncStringDataToScreen(GetResult));
