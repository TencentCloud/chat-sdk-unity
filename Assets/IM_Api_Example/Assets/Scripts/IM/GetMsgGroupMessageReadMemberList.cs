...
public class GetMsgGroupMessageReadMemberList : MonoBehaviour
{
  ...
  public void GetMsgGroupMessageReadMemberListSDK()
  {
    if (SelectedMsg.value > -1 && SelectedMsg.value < MsgList.Count)
    {
      print("Selected Message: " + MsgList[SelectedMsg.value].message_msg_id);
      TIMResult res = TencentIMSDK.MsgGetGroupMessageReadMemberListWeb(MsgList[SelectedMsg.value], (TIMGroupMessageReadMembersFilter)SelectedMemberFilter.value, LastSeq.text, 20, Utils.SetMsgGroupMessageReadMemberListCallbackWeb(GetResult));
