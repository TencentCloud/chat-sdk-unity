...
public class GroupIncreaseGroupCounter : MonoBehaviour
{
  ...
  void Start()
  {
    ...
    Submit.onClick.AddListener(GroupIncreaseGroupCounterSDK);
  ...
  void GroupIncreaseGroupCounterSDK()
  {
    if (groupIDList.Count < 1) return;
    string group_id = groupIDList[SelectedGroup.value];

    TIMResult res = TencentIMSDK.GroupIncreaseGroupCounter(group_id, GroupCounterKey.text, Convert.ToUInt64(GroupCounterValue.text), Utils.addAsyncStringDataToScreen(GetResult));
