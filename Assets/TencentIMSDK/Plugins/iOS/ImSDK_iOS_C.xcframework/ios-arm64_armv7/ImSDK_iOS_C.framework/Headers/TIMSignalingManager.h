 ...
 * 3.1 设置收到信令邀请的回调
 * 
 * @param cb 收到信令邀请的回调，请参考[TIMSignalingReceiveNewInvitationCallback]()
 ...
 * 3.2 设置信令邀请被接收者同意的回调
 * 
 * @param cb 同意信令邀请的回调，请参考[TIMSignalingInviteeAcceptedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 3.3 设置信令邀请被接收者拒绝的回调
 * 
 * @param cb 拒绝信令邀请的回调，请参考[TIMSignalingInviteeRejectedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 3.4 设置信令邀请被取消的回调
 * 
 * @param cb 信令邀请被取消的回调，请参考[TIMSignalingInvitationCancelledCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 3.5 设置信令邀请超时的回调
 * 
 * @param cb 信令邀请超时的回调，请参考[TIMSignalingInvitationTimeoutCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 * 3.6 设置信令邀请被修改的回调
 * 
 * @param cb 信令邀请被修改的回调，请参考[TIMSignalingInvitationModifiedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 */
 ...
 *               }, nullptr);
 */
TIM_DECL int TIMSignalingInvite(const char* invitee, const char* data, bool online_user_only, const char* json_offline_push_info, int timeout, char* invite_id_buffer, TIMCommCallback cb, const void* user_data);
 ...
 *               }, nullptr);
 */
TIM_DECL int TIMSignalingInviteInGroup(const char* group_id, const char* json_invitee_array, const char* data, bool online_user_only, int timeout, char* invite_id_buffer, TIMCommCallback cb, const void* user_data);
 ...
 *
 * @note 
 *  如果 invite 设置 online_user_only 为 false，每次信令操作（包括 invite、cancel、accept、reject、timeout）都会产生一条自定义消息，该消息会通过 @ref [TIMRecvNewMsgCallback](TIMMessageManager.h) 抛给用户，用户也可以通过历史消息拉取，如果需要根据信令信息做自定义化文本展示，可以调用下面接口获取信令信息。
 * 
 * @example
 * const char *json_msg = "";
...
// int [TIMSignalingActionType](), 只读, 信令响应类型
