 ...
 * }
 *
 * @example 返回一个文本消息的Json示例。Json Key请参考[Message]()、[TextElem]()
 * [
 *    {
 ...
 * ]
 *
 * @example 返回一个群通知消息的Json示例。Json Key请参考[Message]()、[GroupReportElem]()
 * [
 *    {
 ...
 * ]
 *
 * @example 返回一个群提示消息的Json示例。Json Key请参考[Message]()、[GroupTipsElem](TIMGroupManager.h)
 * [
 *    {
 ...
 * 2.4 收到消息已读回执
 *
 * @param json_msg_read_receipt_array 消息已读回执数组
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 *
 * @example
 ...
 *
 * @param json_receive_message_option_info 当前用户全局消息接收选项，请参考[TIMReceiveMessageOptInfo]()
 * @param user_data ImSDK负责透传的用户自定义数据，未做任何处理
 */
 ...
 * 3.1 添加接收新消息回调
 * 
 * @param cb 新消息回调函数，请参考[TIMRecvNewMsgCallback]()
 ...
 *
 * @param cb 文件上传进度回调，请参考[TIMMsgElemUploadProgressCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.4 设置消息已读回执回调
 *
 * @param cb  消息已读回执回调，请参考[TIMMsgReadedReceiptCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.5 设置接收的消息被撤回回调
 *
 * @param cb 消息撤回通知回调,请参考[TIMMsgRevokeCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.6 设置消息更新的回调
 *
 * @param cb 消息更新回调，请参考[TIMMsgUpdateCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 * @note
 ...
 * 3.7 设置消息扩展信息更新的回调
 *
 * @param cb 消息扩展信息更新回调,请参考[TIMMsgExtensionsChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 */
 ...
 * 3.8 设置消息扩展信息删除的回调
 * 
 * @param cb 消息扩展信息删除回调,请参考[TIMMsgExtensionsDeletedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 */
 ...
 * 3.9 设置消息回应信息更新的回调
 * 
 * @param cb 消息回应信息更新回调,请参考[TIMMsgReactionsChangedCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 */
 ...
 * 3.10 设置全局消息接收选项的回调
 * 
 * @param cb 消息更新回调，请参考[TIMMsgAllMessageReceiveOptionCallback]()
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 *
 */
 ...
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param message_id  消息 ID
 * @param cb 取消结果的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 ...
 * @param cb 群发消息成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 ...
 * @param path 下载文件保存路径
 * @param cb 下载成功与否的回调以及下载进度回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 ...
 * 在相应元素内找到。其中 kTIMMsgDownloadElemParamType 为下载文件类型 [TIMDownloadType]()，参数 path 要求是 UTF-8 编码
 *
 * @example
 ...
 *
 * @param json_msg_param  消息json字符串
 * @param cb 保存自定义消息成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * @example
 ...
 * @param opt 消息接收选项，请参考[TIMReceiveMessageOpt](TIMCloudComm.h)
 * @param cb 设置成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 *
 * @param json_identifier_array 用户 ID 列表
 * @param cb 查询结果的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * }, this);
 *
 * // 回调的 json_param 示例。Json Key请参考[GetC2CRecvMsgOptResult]()
 * [
 *    {
 ...
 *
 * @param opt                      全局消息接收选项设置，请参考[TIMReceiveMessageOpt](TIMCloudComm.h)
 ...
 *
 */
TIM_DECL int TIMMsgSetAllReceiveMessageOpt(TIMReceiveMessageOpt opt, int32_t start_hour, int32_t start_minute, int32_t start_second, uint32_t duration, TIMCommCallback cb, const void* user_data);
 ...
 *
 */
TIM_DECL int TIMMsgSetAllReceiveMessageOpt2(TIMReceiveMessageOpt opt, uint32_t start_time_stamp, uint32_t duration, TIMCommCallback cb, const void* user_data);
 ...
 * }, this);
 *
 * // 回调的 json_param 示例。Json Key请参考[TIMReceiveMessageOptInfo]()
 * [
 *    {
 ...
 * @param conv_id   会话的ID
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param json_get_msg_param 消息获取参数，请参考 [MsgGetMsgListParam]() Json Key 定义
 * @param cb 获取指定会话的消息列表成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 ...
 * 
 * @example 获取 C2C 会话 Windows-02 消息列表
 ...
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param json_msg_param  消息json字符串
 * @param cb 消息撤回成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * 
 * @param json_msg_param  消息json字符串
 * @param cb 修改消息内容成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 删除指定会话的消息成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param cb 删除消息成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param conv_id 会话的ID
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param cb 清空成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example
 ...
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param json_msg_array  消息数组
 * @param cb 导入消息列表到指定会话成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param cb 根据消息 messageID 查询本地的消息列表成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @example
 ...
 * @param cb 根据消息定位精准查找指定会话的消息成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 * 
 * @note
 ...
 * @param cb 搜索成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @example 搜索本地消息
 ...
 * @param cb 成功与否的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note 请注意：
 ...
 * @param cb 成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 *  <p> 2、调用 TIMMsgRemoveMessageReaction 接口删除已经添加的 emoji，删除成功后，emoji 下就不再存储当前操作用户。
 ...
 *
 */
TIM_DECL int TIMMsgAddMessageReaction(const char *json_msg, const char *reaction_id, TIMCommCallback cb, const void *user_data);
 ...
 *
 */
TIM_DECL int TIMMsgRemoveMessageReaction(const char *json_msg, const char *reaction_id, TIMCommCallback cb, const void *user_data);

/**
 * 6.21 批量拉取多条消息回应信息（7.4 及其以上版本支持，需要您购买旗舰版套餐）
 *
 * @param json_msg_array 消息列表，一次最大支持 20 条消息，消息必须属于同一个会话。
 * @param max_user_count_per_reaction 取值范围 【0,10】，每个 Reaction 最多只返回前 10 个用户信息，如需更多用户信息，可以按需调用 TIMMsgGetMessageReactionUserList 接口分页拉取。
 *
 * @example 批量拉取消息回应列表
 ...
 *
 */
TIM_DECL int TIMMsgGetMessageReactions(const char *json_msg_array, uint32_t max_user_count_per_reaction, TIMCommCallback cb, const void *user_data);
 ...
 *
 */
TIM_DECL int TIMMsgGetAllUserListOfMessageReaction(const char *json_msg, const char *reaction_id, uint32_t next_seq, uint32_t count, TIMCommCallback cb, const void *user_data);
/**
 * 6.23 翻译文本消息
 *
 ...
 * @param cb 翻译结果的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)。
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)。
 * 
 * @example
 ...
 * @param cb 识别结果的回调。回调函数定义和参数解析请参考 [TIMCommCallback](TIMCloudDef.h)。
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)。
 * 
 * @note
...
// uint[TIMImageLevel](), 读写(必填), 发送图片的质量级别
...
// array [Message](), 读写(必填), 消息列表（最大支持 300 条，消息对象必须是 kTIMMsg_SendSucc 状态，消息类型不能为 [GroupTipsElem](TIMGroupManager.h) 或 [GroupReportElem]()）
...
//------------------------------------------------------------------------------
// 7.9 在消息 kTIMMsgGroupAtUserArray 字段中填入 kImSDK_MessageAtALL 表示当前消息需要 @ 群里所有人
static const char* kImSDK_MessageAtALL = "__kImSDK_MessageAtALL__";

//------------------------------------------------------------------------------
// 7.10 GroupTipsElem (群组系统消息元素)
// @note
//  针对所有群成员，可以通过监听 TIMSetGroupTipsEventCallback 获取，请参考 TIMGroupManager.h 中的 GroupTipsElem JsonKey 定义

//------------------------------------------------------------------------------
// 7.11 GroupReportElem (群组系统通知元素)
// @note
//  针对个人，可以通过监听 TIMAddRecvNewMsgCallback 获取
// uint [TIMGroupReportType](), 只读, 类型
...
// object [GroupMemberInfo](), 只读,操作者群内资料
...
// uint [TIMReceiveMessageOpt](TIMCloudComm.h), 只读，消息接收选项, 用户修改群消息接收选项时会收到该信息
...
//     - 此消息在本地不会存储
//  消息自定义字段
//     - 开发者可以对消息增加自定义字段，如自定义整数(通过 kTIMMsgCustomInt 指定)、自定义二进制数据(通过 kTIMMsgCustomStr 指定，必须转换成 String，Json 不支持二进制传输)，可以根据这两个字段做出各种不同效果，例如语音消息是否已经播放等等。另外需要注意，此自定义字段仅存储于本地，不会同步到Server，更换终端获取不到。
...
// uint, [TIMPlatform](), 读写(选填), 发送消息的平台
...
// uint [TIMMsgStatus](), 读写(选填), 消息当前状态
...
// object [UserProfile](), 读写(选填), 消息的发送者的用户资料
static const char* kTIMMsgSenderProfile = "message_sender_profile";
// object [GroupMemberInfo](), 读写(选填), 消息发送者在群里面的信息，只有在群会话有效。目前仅能获取字段 kTIMGroupMemberInfoIdentifier、kTIMGroupMemberInfoNameCard 其他的字段建议通过 TIMGroupGetMemberInfoList 接口获取
...
// object [OfflinePushConfig](), 读写(选填), 消息的离线推送设置
...
// uint [TIMDownloadType](), 只写, 从消息元素里面取出来,元素的类型
...
// object [Message](), 只写(选填), 指定的消息，不允许为null
...
// array uint32 只写(选填)，拉取的消息类型集合，kTIMMsgGetMsgListParamIsRamble 为 false 有效，传入空数组表示拉取全部类型消息，取值详见 TIMElemType。
...
// bool, 读写(必填), 要查找的消息是否是被撤回。true表示被撤回的，false表示未撤回的。默认为false
...
// bool, 读写(必填), 要查找的消息的发送者是否是自己。true表示发送者是自己，false表示发送者不是自己。默认为false
...
// array [TIMElemType](), 只写(选填), 指定搜索的消息类型集合，传入空数组，表示搜索支持的全部类型消息（[FaceElem]() 和 [GroupTipsElem](TIMGroupManager.h) 暂不支持）取值详见 TIMElemType。
...
// uint [TIMConvType](TIMCloudComm.h), 只写(选填), 会话类型，如果设置 kTIMConv_Invalid，代表搜索全部会话。否则，代表搜索指定会话。
...
// uint64, 只写(选填), 从起始时间点开始的过去时间范围，单位秒。默认为0即代表不限制时间范围，传24x60x60代表过去一天。
...
// string, 只写(选填)，服务武器搜索游标。第一次填空字符串，续拉时填写 [MessageSearchResult]() 中的返回值。
...
// uint [TIMConvType](TIMCloudComm.h), 只读, 会话类型
...
// uint 只读，[TIMReceiveMessageOpt](TIMCloudComm.h), 消息接收选项，支持两种类型：kTIMRecvMsgOpt_Receive 和 kTIMRecvMsgOpt_Not_Notify
...
// array [UserInfo](), 只读, 使用同一个 reaction_id 回应消息的部分用户列表（用户列表数量取决于调用 TIMMsgGetMessageReactions 接口时设置的 max_user_count_per_reaction 值）
...
// array [MessageReaction](), 只读, 消息回应列表
...
// array [MessageReaction](), 只读, 变更的消息回应列表
 ...
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param json_msg_param  消息json字符串
 * @param msgid_cb 返回消息 ID 的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param res_cb 发送新消息成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
 ...
 * @param conv_type 会话类型，请参考[TIMConvType](TIMCloudComm.h)
 * @param json_msg_param  消息json字符串
 * @param cb 消息上报已读成功与否的回调。回调函数定义请参考 [TIMCommCallback](TIMCloudDef.h)
 * @param user_data 用户自定义数据，ImSDK只负责传回给回调函数cb，不做任何处理
 * @return int 返回TIM_SUCC表示接口调用成功（接口只有返回TIM_SUCC，回调cb才会被调用），其他值表示接口调用失败。每个返回值的定义请参考 [TIMResult](TIMCloudDef.h)
 *
 * @note
